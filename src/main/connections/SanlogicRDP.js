...
function arrayMatchTransform(array, increment, def) {
  return function (value) {
    var index;
    if (ObjectUtils.isUndefined(increment)) {
      increment = 0;
    }
    ...
    if (ObjectUtils.isUndefined(def)) {
 ...
 */
export default class SanlogicRDP extends EventEmitter {
  ...
  setIdIfNotSpecified(connectionId){
  ...
  connect(config, filePath) {
    ...
    let defaultCommandExe = 'mstsc.exe'

    //获取用户配置的mstsc应用
    let userCommandExe = ConfigTool.get(ConfigConstant.KEY_CUSTOMER_APP_MSTSC)
    if(userCommandExe && fs.existsSync(userCommandExe) && fs.statSync(userCommandExe).isFile()){
      defaultCommandExe = userCommandExe
    }

    // console.log(filePath)
    // this.proc = spawn('mstsc.exe', [filePath, '/admin', '/v', config.address]);
    // this.proc = spawn('mstsc.exe', [filePath, '/console', '/v', config.address]);
    this.proc = spawn(defaultCommandExe, [filePath, '/console', '/v', config.address]);
  ...
  init() {
    ...
    this.generateFile(this.params).then(filePath => {
      ...
      this.emit('error','Generate File Error!')
    });
  }
}
